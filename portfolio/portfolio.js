...
(function () {
  ...
  if (canvas && typeof THREE !== 'undefined') {
    try {
      ...
      scene.fog = new THREE.FogExp2(0x18280c, 0.055);
      ...
      var gold = new THREE.Color(0xd9a441);
      var green = new THREE.Color(0x688f33);
      ...
      for (var r = 0; r < ROWS; r++) {
        for (var c = 0; c < COLS; c++) {
          ...
          positions[i * 3 + 1] = 0;
          positions[i * 3 + 2] = z;
          // far rows fade from straw gold into field green
          tmp.copy(gold).lerp(green, r / (ROWS - 1));
      ...
      function render() {
        ...
        // occasional stronger gusts roll across the field
        var gust = 1 + 0.4 * Math.sin(t * 0.21) * Math.sin(t * 0.083 + 2.1);
        for (var j = 0; j < count; j++) {
          pos[j * 3 + 1] = wave(pos[j * 3], pos[j * 3 + 2], t) * gust;
        }
